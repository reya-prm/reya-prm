<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=FF2D20&height=160&section=header&text=Reya%20Permana&fontSize=44&fontColor=fff&animation=fadeIn&fontAlignY=32&desc=Laravel%20Developer&descAlignY=62" />
</div>

<br/>

<div style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,45,32,0.25); border-radius: 16px; padding: 24px; backdrop-filter: blur(8px);">

## About Me

> "Code is like humor. When you have to explain it, it's bad."

**Laravel Developer** â€” Crafting clean controllers & views.

- Focus: Clean Architecture, POS Systems, Blade & Tailwind
- Stack: Laravel, PHP, MySQL, Tailwind, Vite
- Learning: Eloquent Relations, Filament, Best Practices
- Email: reyapermana122@gmail.com
- Fun: Ngoding sambil dengerin musik

<details>
<summary>PHP snippet</summary>

```php
class Reya extends Developer {
    public string $role = "Laravel Developer";
    public string $motto = "Less talk, more artisan migrate";
}
```

</details>

</div>

<br/>

## Skills

<p align="center">
  <img src="https://skillicons.dev/icons?i=laravel,php,mysql,tailwind,js,html,css,git,vscode,github" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Laravel-FF2D20?logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/PHP-777BB4?logo=php&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/Tailwind-06B6D4?logo=tailwindcss&logoColor=white" />
</p>

## GitHub Stats

<div align="center">



<br/><br/>

<img src="https://streak-stats.demolab.com?user=reya-prm&theme=tokyonight&hide_border=true&background=0d1117&ring=FF2D20&fire=FF2D20&currStreakLabel=FF2D20" />

</div>
